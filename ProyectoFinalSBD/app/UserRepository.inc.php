...
class UserRepository {
	...
	public static function updateEmail($conexion, $username, $newEmail){
		$user = null;
	...
	public static function updatePassword($conexion, $username, $newPass){
		$user = null;
		$newPass2 = password_hash($newPass, PASSWORD_DEFAULT);
		
		if(isset($conexion)){
			try{
				$sql = 'UPDATE users SET password = :newPass2 WHERE username = :username';
				$statement = $conexion->prepare($sql);
				$statement->bindParam(':newPass2', $newPass2, PDO::PARAM_STR);
				$statement->bindParam(':username', $username, PDO::PARAM_STR); 
				$statement -> execute();
				echo "SI JALO";
			}catch (Exception $ex) {
